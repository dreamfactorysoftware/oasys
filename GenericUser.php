 ...
 */
class GenericUser extends Seed
{
	//*************************************************************************
	//* Members
	//*************************************************************************
	...
	/**
	 * @var string The provider's ID for this dude
	 */
	...
	/**
	 * @var bool Boolean value indicating whether the user and this Contact have established a bi-directionally
	 * asserted connection of some
	 * kind on the Service Provider's service. The value MUST be either true or false. The value MUST be true
	 * if and only if there is at
	 * least one value for the relationship field, described below, and is thus intended as a summary value
	 * indicating that some type of
	 * bi-directional relationship exists, for Consumers that aren't interested in the specific nature of that
	 * relationship. For traditional
	 * address books, in which a user stores information about other contacts without their explicit
	 * acknowledgment,
	 * or for services in which users choose to "follow" other users without requiring mutual consent,
	...
	public static function fromOpenSocialFeed( $feed )
	{
		...
		foreach ( $feed->entry as $_key => $_entry )
		{
			 ...
			 * System fields
			 */
			$_contact->providerContactId = $_entry->id;
			$_contact->provider = 'opensocial';
			$_contact->parentId = $_userId;
	...
	/**
	 * Parses a Google Data "entry" into a \stdClass
	 *
	...
	{
		//	Comes from google, but uses zend format
		return self::fromZendContact( $entry, $contact );
	}
	 ...
	 * @param GenericUser $contact A source contact
	 *
	 * @return GenericUser
	 */
	public static function fromZendContact( $entry, $contact = null )
	{
		$_contact = $contact ? : new GenericUser();

		$_contact->setparentId = T::app( T::UserId );
		$_contact->setproviderContactId = $entry->getContactId();
		$_contact->setprovider = 'google';
		$_contact->setdeleted = false;
		$_contact->setlinked = false;
		$_contact->setemailAddress = (string)$entry->getPrimaryEmail();
		$_contact->setetag = (string)$entry->getEtag();
		$_contact->setgender = (string)$entry->getGenderValue();
		$_contact->setnickname = (string)$entry->getNickname();
		$_contact->setextendedProperties = \Kisma\Utility\Transform::xmlToArray2( $entry->getExtendedProperties() );
		$_contact->setrawSource = $entry->saveXML();
		$_contact->setupdated = (string)$entry->updated;
		$_contact->setpublished = (string)$entry->published;
		$_contact->setemployer = array(
			'title'       => (string)$entry->getOrgTitle(),
			'companyName' => (string)$entry->getOrgName(),
		);

		$_name = $entry->getName();
		$_contact->setname = is_object( $_name ) ? $_name->toArray() : $_name;

		if ( empty( $_contact->setemails ) )
		{
			$_contact->setemails = array();
		}

		foreach ( $entry->getEmails() as $_email )
		{
			$_address = $_email->toArray();
			$_contact->setemails[$_address['address']] = $_address;
		}

		if ( empty( $_contact->setphoneNumbers ) )
		{
			$_contact->setphoneNumbers = array();
		}

		foreach ( $entry->getphoneNumbers() as $_phone )
		{
			$_contact->setphoneNumbers[$_phone->getText()] = $_phone->toArray();
		}

		if ( empty( $_contact->setgroupMemberships ) )
		{
			$_contact->setgroupMemberships = array();
		}

		foreach ( $entry->getGroupMembershipInfo() as $_group )
		{
			$_contact->setgroupMemberships[] = (string)$_group;
		}

		if ( null !== ( $_org = $entry->getOrganization() ) )
		{
			$_contact->setorganizations[] = $_org->toArray();
		}
	...
	/**
	 * Parses an atomic entry into a GenericUser
	 *
	 * @param array $profile
	 *
	 * @return $this
	 */
	public static function fromFacebook( $profile )
	{
		$_contact = new GenericUser();

		$_name = array(
			'formatted'  => Option::get( $profile, 'name' ),
			'familyName' => Option::get( $profile, 'last_name' ),
			'givenName'  => Option::get( $profile, 'first_name' ),
		);

		$_contact->setId( 'facebook:' . Option::get( $profile, 'id' ) )->setPublished( Option::get( $profile, 'updated_time' ) )
		->setUpdated( Option::get( $profile, 'updated_time' ) )->setDisplayName( $_name['formatted'] )
		->setName( $_name )->setPreferredUsername( Option::get( $profile, 'username' ) )->setGender(
				Option::get(
					$profile,
					'gender'
				)
			)->setEmails( array( Option::get( $profile, 'email' ) ) )->setUrls(
				array(
					 Option::get( $profile, 'link' )
				)
			)->setRelationships( Option::get( $profile, 'friends' ) );
	...
	{
		//	Make sure display name is set before we save...
		if ( empty( $this->_displayName ) )
		{
			if ( !empty( $this->_name['fullName'] ) )
			{
				$this->_displayName = $this->_name['fullName'];
			}
			else
			{
				$this->_displayName = $this->_name['fullName'] =
					$this->_name['givenName'] . ' ' . $this->_name['familyName'];
			}
		}
	 ...
	 * @param mixed $contact
	 *
	 * @throws NNotImplementedException
	 */
